...
def get_status():
    ...
    result = subprocess.run(["systemctl", "status", "openvpn@server"], capture_output=True, text=True, check=True)
...
def build_client(name: str): 
    ...
    logger.info(f"Adding client {name}...")
    ...
    if result.returncode == 0:
        logger.info(f"Client {name} has been successfully added to OpenVPN server.")
        return True
    else:
        logger.error(f"Failed to add client {name} to OpenVPN server.")
        return False
...
def add_client(name: str, ip: str):
    ...
    logger.info(f"Client {name} with IP address {ip} has been successfully added to OpenVPN server.")
...
def list_connections():
    ...
    with open("/var/log/openvpn/openvpn-status.log", "r") as file:
        ...
        logger.info(f"Found {len(connections)} connection(s) from OpenVPN server.")
        return connections
